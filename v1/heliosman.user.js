...
// @icon            https://www.imageoss.com/images/2021/07/01/heliosmaned7457a4c5fd0de8.th.png
...
(function () {
    ...
    function isJp() {
        ...
        return origin.indexOf("spendia.jp") != -1;
    }

    function getWebToken() {
        let accountTokenInfo;
        if (isJp()) {
            accountTokenInfo = sessionStorage.getItem("hly.token");
        } else {
            accountTokenInfo = localStorage.getItem("hly.token");
        }
     ...
     * @param accountToken
     */
    function getTenantId(accountToken) {
        let origin = window.location.origin;
        let tenantIdDetails = {
            "method": "GET",
            ...
            "onload": function (resp) {
                ...
                if (empty(GM_getValue("tenantName"))) {
                    getTenantName(accountToken, tenantId);
                }
            }
        }
    ...
    /**
     * 获取租户名称
     * @param accountToken  接口请求Token
     * @param tenantId  租户ID
     */
    function getTenantName(accountToken, tenantId) {
        let origin = window.location.origin;
    ...
    function refreshTenantInfo() {
        ...
        if (accountToken) {
            if (empty(GM_getValue("tenantId"))) {
                getTenantId(accountToken);
            }
            if (empty(GM_getValue("tenantName")) && empty(GM_getValue("tenantId"))) {
                getTenantName(accountToken, GM_getValue("tenantId"));
            }
        }
    }
    ...
    function initHeliosman() {
        let heliosmanCSS = "#heliosman{position:absolute;top:50%;right:-10px;width:80px;height:80px;border:0;z-index:99999999}.circle{position:absolute;top:50%;left:50%;background:#13a713;color:white;font-size:20px;width:56px;text-align:center;height:56px;line-height:56px;border-radius:50%;box-shadow:0 2px 8px 0 rgb(0 0 0 / 20%);transition:all .08s;user-select:none;-moz-user-select:none;-ms-user-select:none;-webkit-user-select:none;transform:translate3d(-50%,-50%,0)}.tooltip{display:inline-block;position:relative;border-bottom:1px dotted #666;text-align:left}.tooltip h3{margin:12px 0}.tooltip .left{min-width:500px;cursor:auto;white-space:nowrap;text-align:left;top:50%;right:100%;margin-right:5px;transform:translate(0,-50%);padding:10px;background-color:#fff;background-clip:padding-box;border-radius:4px;font-weight:normal;color:rgba(0,0,0,.65);font-size:13px;position:absolute;box-sizing:border-box;display:none;border:1px solid #bfbfbf;box-shadow:0 4px 8px 0 rgb(0 0 0 / 15%)}.tooltip:hover .left{display:block}.tooltip .left div{height:24px;line-height:24px;font-size:16px;font-weight:500}.tooltip .left ul li{height:24px;line-height:24px;font-size:14px;font-weight:200}.tooltip .left ul li span{height:24px;line-height:24px;font-size:14px;font-weight:400}.tooltip .left i{position:absolute;top:50%;left:100%;margin-top:-12px;width:12px;height:24px;overflow:hidden}.tooltip .left i::after{content:'';position:absolute;width:12px;height:12px;left:0;top:50%;transform:translate(-50%,-50%) rotate(-45deg);background-color:#fff;border:1px solid #bfbfbf}.tooltip .copy{position:absolute;right:10px;cursor:pointer;font-size:14px;font-weight:500}";
